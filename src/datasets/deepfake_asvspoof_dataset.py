...
class DeepFakeASVSpoofDataset(SimpleAudioFakeDataset):
    ...
    def add_line_to_samples(self, samples, line):
        ...
        if len(parts) < 8:
            print(f"Warning: Skipping malformed line (too few parts): {line}")
            return samples
        
        sample_name = parts[1]
        label = parts[5]
        
        # Check if the sample name exists in flac_paths
        if sample_name not in self.flac_paths:
            print(f"Warning: Sample {sample_name} not found in flac_paths. Skipping.")
            return samples
        
        sample_path = self.flac_paths[sample_name]
        if not sample_path.exists():
            print(f"Warning: File not found for sample {sample_name}. Skipping.")
            return samples
